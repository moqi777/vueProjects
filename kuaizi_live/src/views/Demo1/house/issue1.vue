...
<!-- 房源图片 -->
<span style="margin: 20px;">房源图片</span>
<div class="upload">
    <van-uploader v-model="fileList" multiple :max-count="9"/>
    <br><span>点击上传图片，最多9张</span>
</div>
<!-- 房源详情 -->
...
const onConfirm5 = ({ selectedOptions }) => {
    result5.value = selectedOptions[0]?.text;
    house.value.property.paymentMethod = selectedOptions[0]?.value;
...
const submit=()=>{
  ...
  if (house.value.contact.phone === '' || house.value.contact.phone === undefined) {
    ...
    api.postReq('/khouse/addIssue1',formData,{
        headers: {
